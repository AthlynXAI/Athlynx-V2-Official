...
| Supabase Auth | `server/_core/supabaseAdmin.ts` | `SUPABASE_URL` (defaults to `"athlynxai"`) | **LIVE** | Uses Google JWKS endpoint for token verification — no service account file needed. Project ID hardcoded as `athlynxai`. Mobile app uses Supabase Auth for auth. |
