...
### Permanent Monday / Metabase / Calendly Connector Proof Gate

Chad provided connector-screen proof that **Monday.com**, **Metabase**, and **Calendly** were visibly OFF while many surrounding connectors remained ON. The operating conclusion is not that Chad disabled them. The operating conclusion is that connector state can drift, expire, fail health checks, or become non-callable selectively, and AthlynXAI OS must never proceed from memory or stale configuration again.

Every new session, scheduled job, connector audit, deployment verification, social workflow, calendar workflow, CRM workflow, analytics workflow, or database workflow must apply a two-layer connector proof gate before work begins:

1. **Visible ON proof:** every connector required by the lane must be visibly ON in the current connector screen or current connector configuration.
2. **Harmless live capability proof:** every connector required by the lane must pass a read-only or no-op capability check against the exact account, server, project, board, workspace, or API surface needed for that lane.

Configuration memory, a prior successful session, a generic enabled flag, or screenshots from a prior time is not enough. If a connector is visible OFF, missing, stale, disconnected, not callable, returns `server not found`, returns `AUTH_FAILED`, returns missing scopes, or fails an MCP/tool invocation, the connector is **blocked** for that lane. Stop immediately and report the connector name, visible state, callable-check result, exact safe reconnect step, and work blocked. Do not push, deploy, upload, send, post, schedule Buffer, trigger Zapier, create Jira/Confluence records, mutate databases, edit secrets, or perform payments until the required connector passes both checks in the same session.
...
| Monday.com | Work management and operating-board candidate. | Visible ON plus harmless read-only board/workspace proof required before any Monday-backed work; no board/item mutations without approval. |
| Metabase | Analytics/dashboard candidate for operating intelligence. | Visible ON plus harmless read-only metadata/dashboard proof required before any analytics claims; no dashboard/query mutations without approval. |
| Calendly | Scheduling and booking rail candidate. | Visible ON plus harmless read-only event-type/profile proof required before any scheduling flow; no event-type or booking changes without approval. |
