...
// drizzle/schema.ts
var userRoleValues = ["user", "admin"];
var nilDealStatusValues = [
  "pending",
  "active",
  "completed",
  "declined"
];
var transferStatusValues = [
  "entered",
  "committed",
  "withdrawn"
];
var verifTypeValues = ["signup", "login", "password_reset"];
var postTypeValues = [
  "status",
  "achievement",
  "workout",
...
var crmContactStatusValues = [
  "Lead",
  "Active",
  "VIP",
  "Churned"
];
var crmPipelineStageValues = [
  "New Lead",
  "Contacted",
  "Demo Scheduled",
  "Proposal Sent",
...
var notifPriorityValues = ["low", "normal", "high", "urgent"];
var postVisibilityValues = ["public", "followers", "private"];
var postMediaTypeValues = [
  "none",
  "image",
  "video",
  "gallery"
];
var postSourceValues = [
  "nil_portal",
  "diamond_grind",
  "messenger",
  "transfer_portal",
...
var convTypeValues = ["direct", "group"];
var convParticipantRoleValues = ["member", "admin"];
var userRoleEnum = pgEnum("user_role", userRoleValues);
var nilDealStatusEnum = pgEnum("nil_deal_status", nilDealStatusValues);
var transferStatusEnum = pgEnum(
  "transfer_status",
  transferStatusValues
);
var verifTypeEnum = pgEnum("verif_type", verifTypeValues);
var postTypeEnum = pgEnum("post_type", postTypeValues);
var crmContactRoleEnum = pgEnum(
  "crm_contact_role",
  crmContactRoleValues
);
var crmContactStatusEnum = pgEnum(
  "crm_contact_status",
  crmContactStatusValues
);
var crmPipelineStageEnum = pgEnum(
  "crm_pipeline_stage",
  crmPipelineStageValues
);
var notifTypeEnum = pgEnum("notif_type", notifTypeValues);
var notifPriorityEnum = pgEnum("notif_priority", notifPriorityValues);
var postVisibilityEnum = pgEnum(
  "post_visibility",
  postVisibilityValues
);
var postMediaTypeEnum = pgEnum("post_media_type", postMediaTypeValues);
var postSourceEnum = pgEnum("post_source", postSourceValues);
var msgTypeEnum = pgEnum("msg_type", msgTypeValues);
var convTypeEnum = pgEnum("conv_type", convTypeValues);
var convParticipantRoleEnum = pgEnum(
...
var athleteProfiles = pgTable("athlete_profiles", {
  id: serial("id").primaryKey(),
  ...
  // Athlete playlist / podcast
  capcutUrl: text("capcutUrl"),
  // CapCut highlight reel
  // ─── Build 2: profile presentation columns ──────────────────────────────
  headshotUrl: text("headshotUrl"),
  // Cloudflare R2 headshot URL
  actionPhotoUrl: text("actionPhotoUrl"),
  // R2 action shot for hero bar
  statScreenshotUrl: text("statScreenshotUrl"),
  // R2 stat sheet upload (onboarding step 7)
  jerseyNumber: smallint("jerseyNumber"),
  // 0-99, displayed in gold on hero
  dominantHand: varchar("dominantHand", { length: 16 }),
  // "Right" | "Left" | "Both" | "R/R" | "L/R" etc.
  athlynxStarRating: smallint("athlynxStarRating"),
  // 1-5, manually awarded
  athleticismScore: real("athleticismScore"),
  // PlayerProfiler-style 0-100
  published: boolean("published").default(false).notNull(),
  // Onboarding step 10 flips this true
  publishedAt: timestamp("publishedAt"),
  // ─── Build 2: structured recruiting ──────────────────────────────────────
  recruitingTopFive: json("recruitingTopFive"),
  // Array of 5 college names locked at Signing Day onboarding step
  // ─── Build 3: editable everywhere + avatar library + ladder direction ──
  avatarChoiceKey: varchar("avatarChoiceKey", { length: 64 }),
  // slug into sports_avatar_library when athlete picks a library avatar instead of uploading
  coverChoiceKey: varchar("coverChoiceKey", { length: 64 }),
  // same idea for cover photo library
  recruitingLadderDirection: varchar("recruitingLadderDirection", { length: 16 }).default("countdown").notNull(),
  // 'countdown' | 'topdown'
  fieldPrivacy: json("fieldPrivacy"),
  // { gpa:false, weight:false ... } for per-field hide-from-public toggles
...
var nilDeals = pgTable("nil_deals", {
  id: serial("id").primaryKey(),
...
var transferPortalEntries = pgTable("transfer_portal_entries", {
  id: serial("id").primaryKey(),
  athleteId: integer("athleteId").notNull(),
...
var waitlist = pgTable("waitlist_entries", {
  id: serial("id").primaryKey(),
  email: varchar("email", { length: 320 }).notNull(),
  name: varchar("name", { length: 255 }),
...
var crmContacts = pgTable("crm_contacts", {
  id: serial("id").primaryKey(),
...
var broadcastChannelEnum = pgEnum("broadcast_channel", [
  "email",
  "in_app",
  "both"
]);
var broadcastRecipientEnum = pgEnum("broadcast_recipient", [
  "all",
  "trial",
  "subscribed",
  "free"
]);
var broadcastStatusEnum = pgEnum("broadcast_status", [
  "draft",
  "sent",
  "failed"
]);
...
var feedbackStatusEnum = pgEnum(
  "feedback_status",
  feedbackStatusValues
);
var feedbackCategoryEnum = pgEnum(
  "feedback_category",
  feedbackCategoryValues
);
var athleteFeedback = pgTable("athlete_feedback", {
  id: serial("id").primaryKey(),
  userId: integer("userId"),
...
var expiryNoticeStatusValues = ["sent", "failed", "skipped"];
var expiryEmailTypeEnum = pgEnum(
  "expiry_email_type",
  expiryEmailTypeValues
);
var expiryNoticeStatusEnum = pgEnum(
  "expiry_notice_status",
  expiryNoticeStatusValues
);
var subscriptionExpiryNotices = pgTable(
...
var dataSourceTypeEnum = pgEnum(
  "data_source_type",
  dataSourceTypeValues
);
var dataEventTypeEnum = pgEnum("data_event_type", dataEventTypeValues);
var athleteDataSources = pgTable("athlete_data_sources", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
...
var athleteDataEvents = pgTable("athlete_data_events", {
  id: serial("id").primaryKey(),
...
var athleteDataSummaries = pgTable("athlete_data_summaries", {
  id: serial("id").primaryKey(),
  athleteId: integer("athleteId").notNull(),
...
var creditTxTypeEnum = pgEnum("credit_tx_type", creditTxTypeValues);
...
var creditPackagePurchases = pgTable("credit_package_purchases", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
...
var aiTrainerRoleValues = ["user", "assistant"];
var aiTrainerRoleEnum = pgEnum("ai_trainer_role", aiTrainerRoleValues);
var aiTrainerSessions = pgTable("ai_trainer_sessions", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
...
var calendarEventTypeEnum = pgEnum(
  "calendar_event_type",
  calendarEventTypeValues
);
var athleteCalendarEvents = pgTable("athlete_calendar_events", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
  title: varchar("title", { length: 255 }).notNull(),
...
var athleteStories = pgTable("athlete_stories", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
...
var licenseStatusEnum = pgEnum("license_status", licenseStatusValues);
var licenseTierValues = [
  "team",
  "school",
  "conference",
  "enterprise"
];
var licenseTierEnum = pgEnum("license_tier", licenseTierValues);
var licenseAgreements = pgTable("license_agreements", {
  id: serial("id").primaryKey(),
  orgName: varchar("orgName", { length: 255 }).notNull(),
  orgType: varchar("orgType", { length: 64 }).notNull(),
  tier: licenseTierEnum("tier").notNull(),
  monthlyFee: integer("monthlyFee").notNull(),
  // in cents
...
var recruitingInterestLevelValues = [
  "watching",
  "interested",
  "offer_extended",
  "committed"
];
var recruitingInterestLevelEnum = pgEnum(
  "recruiting_interest_level",
  recruitingInterestLevelValues
);
var recruitingLetterTypeValues = [
  "offer",
  "commitment",
  "interest",
  "ncaa_loi",
...
var recruitingBoardEntries = pgTable("recruiting_board_entries", {
  ...
  rank: smallint("rank"),
  // 1-5, the slot in the Top 5 ladder. null = not in top 5
  letterUrl: text("letterUrl"),
  // R2 URL of uploaded offer/commitment/LOI
...
var recruitingCoachViews = pgTable("recruiting_coach_views", {
  id: serial("id").primaryKey(),
  athleteId: integer("athleteId").notNull(),
  coachName: varchar("coachName", { length: 128 }),
...
var awardCategoryEnum = pgEnum("award_category", awardCategoryValues);
var athleteAwards = pgTable("athlete_awards", {
  id: serial("id").primaryKey(),
  athleteId: integer("athleteId").notNull(),
  title: varchar("title", { length: 255 }).notNull(),
  ...
  // hex or gold/silver/bronze
  verified: boolean("verified").default(false).notNull(),
  proofUrl: text("proofUrl"),
  // R2-hosted proof image / news article
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
var socialPlatformValues = [
  "instagram",
  "tiktok",
  "twitter",
  "youtube",
...
var socialPlatformEnum = pgEnum(
  "social_platform",
  socialPlatformValues
);
var athleteSocialAccounts = pgTable("athlete_social_accounts", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
...
var coachLynxRoleValues = ["system", "user", "assistant"];
var coachLynxRoleEnum = pgEnum("coach_lynx_role", coachLynxRoleValues);
var coachLynxMessages = pgTable("coach_lynx_messages", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
  role: coachLynxRoleEnum("role").notNull(),
  content: text("content").notNull(),
  contextScreen: varchar("contextScreen", { length: 64 }),
  // which page they were on
  modelUsed: varchar("modelUsed", { length: 64 }),
  // "nebius" | "gemini" | "claude" | "openai"
  tokensIn: integer("tokensIn").default(0),
  tokensOut: integer("tokensOut").default(0),
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
var coachLynxMemory = pgTable("coach_lynx_memory", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
  fact: text("fact").notNull(),
  // "User competes at 6A in Texas"
  source: varchar("source", { length: 64 }),
  // "onboarding" | "chat" | "manual"
  confidence: real("confidence").default(1),
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
var founderFollows = pgTable("founder_follows", {
  id: serial("id").primaryKey(),
  founderUserId: integer("founderUserId").notNull(),
  athleteId: integer("athleteId").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
var founderNotes = pgTable("founder_notes", {
  id: serial("id").primaryKey(),
  founderUserId: integer("founderUserId").notNull(),
  athleteId: integer("athleteId").notNull(),
...
var founderLessons = pgTable("founder_lessons", {
  id: serial("id").primaryKey(),
  founderUserId: integer("founderUserId").notNull(),
...
var brandTierEnum = pgEnum("brand_tier", brandTierValues);
var brandPursuitStatusValues = [
  "wishlist",
  "pitched",
  "conversation",
  "signed",
...
var brandPursuitStatusEnum = pgEnum(
  "brand_pursuit_status",
  brandPursuitStatusValues
);
var brandWallEntries = pgTable("brand_wall_entries", {
  id: serial("id").primaryKey(),
...
var brandWallPursuits = pgTable("brand_wall_pursuits", {
  id: serial("id").primaryKey(),
  athleteId: integer("athleteId").notNull(),
...
var sportsAvatarLibrary = pgTable("sports_avatar_library", {
  id: serial("id").primaryKey(),
  ...
  // 'football' | 'basketball' | etc, NULL = universal
...
var podcastEpisodeGuests = pgTable("podcast_episode_guests", {
  id: serial("id").primaryKey(),
...
var streamProviderValues = [
  "cloudflare_stream",
  "mux",
  "youtube",
  "vimeo"
];
var streamProviderEnum = pgEnum("stream_provider", streamProviderValues);
var streamStatusValues = [
  "scheduled",
  "live",
  "ended",
  "vod",
...
var streamStatusEnum = pgEnum("stream_status", streamStatusValues);
...
var streamEvents = pgTable("stream_events", {
  id: serial("id").primaryKey(),
...
var _db = null;
var _pool = null;
var _lastConnectAttempt = 0;
var RECONNECT_COOLDOWN_MS = 5e3;
async function getDb() {
  ...
  const urls = [
    ...
    { url: (process.env.DATABASE_URL || "").trim(), label: "DATABASE_URL fallback" },
    { url: (process.env.PLANETSCALE_DATABASE_URL || "").trim(), label: "PlanetScale" }
  ].filter((u, index, all) => u.url && all.findIndex((candidate) => candidate.url === u.url) === index);
...
// server/jobs/ownerDailyPost.ts
init_drizzle_orm();

// server/services/valuesGate.ts
var FORBIDDEN_WORDS = [
  /\bfaster\b/i,
  /\bsmarter\b/i,
  /\bbetter than\b/i,
...
function runValuesGate(text2) {
  ...
  let rewrite_hint;
  if (!passes) {
    rewrite_hint = "Rewrite in Athlynx voice: man on a porch telling the truth. Honor one athlete or one truth about the journey one identity to every platform. No specs, no comparisons, no AI-powered talk, no emojis. Match at least one of the five core values.";
  }
  return { passes, reasons, matched_values, rewrite_hint };
}

// server/jobs/ownerDailyPost.ts
var VOICE_BANK_REAL_AI = [
  "Athlynx isn't a recruiting app. It's a place to be known. Stand still long enough and the right people will find you.",
  "ONE IDENTITY. EVERY ATHLETE. EVERY PLATFORM. That isn't a slogan. It's the floor.",
...
async function runOwnerDailyPostCron() {
  ...
  try {
    const autoRows = await db.execute(sql`
      ...
      WHERE enabled = true
    `);
    const rows = autoRows.rows ?? autoRows ?? [];
    ...
    for (const row of rows) {
      try {
        ...
        const body = voice === "real_me" ? pick(VOICE_BANK_REAL_ME) : pick(VOICE_BANK_REAL_AI);
        const gate = runValuesGate(body);
        if (!gate.passes) {
          skipped++;
          continue;
        }
        const scheduledFor = new Date(Date.now() + 15 * 60 * 1e3);
...
// server/cron-src/owner-daily-post.ts
