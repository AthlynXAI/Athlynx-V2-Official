...
function ReelMastersInner() {
  ...
  return (
    <div className="min-h-screen bg-[#001520]">
      {/* Header */}
      <header className="sticky top-0 z-50 bg-[#001520]/95 backdrop-blur border-b border-[#1E90FF]/30">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs text-[#00C2FF] font-bold tracking-widest">REEL MASTERS</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/signin" className="bg-[#1565C0] hover:bg-[#1E90FF] text-white text-sm font-bold px-4 py-2 rounded-lg transition-colors">
              Join Tournament
            </Link>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden py-20 px-4">
        <div className="absolute inset-0 bg-gradient-to-br from-[#1E90FF] via-[#001520] to-blue-950 opacity-70" />
        <div className="relative max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-[#1E90FF]/20 border border-[#1E90FF]/30 rounded-full px-4 py-1.5 mb-6">
            <span className="text-2xl"></span>
            <span className="text-[#00C2FF] text-xs font-bold tracking-widest">REEL MASTERS</span>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/signin" className="flex items-center justify-center gap-2 bg-[#1565C0] hover:bg-[#1E90FF] text-white font-bold text-lg px-8 py-4 rounded-xl transition-all hover:scale-105">
               Join Tournament
            </Link>
            <button className="flex items-center justify-center gap-2 border border-[#1E90FF] text-[#00C2FF] hover:bg-[#1E90FF]/20 font-bold text-lg px-8 py-4 rounded-xl transition-all">
               Find Fishing Spots
            </button>
          </div>
        </div>
      ...
      {/* Stats */}
      <section className="py-10 px-4 border-y border-[#1E90FF]/30 bg-black/30">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <div className="text-3xl mb-1">{s.icon}</div>
              <div className="text-3xl font-black text-[#00C2FF]">{s.value}</div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          {activeTab === "overview" && (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {features.map((f) => (
                <div key={f.title} className="bg-white/5 border border-[#1E90FF]/30 rounded-xl p-5 hover:border-[#1E90FF]/30 transition-all">
          ...
          {activeTab === "tournaments" && (
            <div className="space-y-4">
              {[
                ...
                <div key={t.name} className="bg-white/5 border border-[#1E90FF]/30 rounded-xl p-5 flex items-center justify-between hover:border-[#1E90FF]/30 transition-all">
                  <div>
                    <div className="text-white font-bold">{t.name}</div>
                    <div className="text-gray-400 text-sm">{t.date} · {t.location}</div>
                    <span className="text-xs bg-[#1E90FF]/20 text-[#00C2FF] px-2 py-0.5 rounded-full mt-1 inline-block">{t.type}</span>
                  </div>
                  <div className="text-right">
                    <div className="text-[#00C2FF] font-bold">{t.prize}</div>
                    <button className="mt-2 bg-[#1565C0] hover:bg-[#1E90FF] text-white text-xs font-bold px-4 py-1.5 rounded-lg transition-colors">
                      Register
                    </button>
                  </div>
                </div>
          ...
          {activeTab === "spots" && (
            <div className="grid md:grid-cols-2 gap-5">
              {[
                ...
                <div key={spot.name} className="bg-white/5 border border-[#1E90FF]/30 rounded-xl p-5 hover:border-[#1E90FF]/30 transition-all">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-right">
                      <div className="text-[#1E90FF] font-bold"> {spot.rating}</div>
                      <div className="text-gray-500 text-xs">{spot.reports} reports</div>
                    </div>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    {spot.species.map((s) => (
                      <span key={s} className="bg-[#1E90FF]/20 text-[#00C2FF] text-xs px-2 py-0.5 rounded">{s}</span>
                    ))}
                  </div>
                  <button className="mt-4 w-full border border-[#1E90FF] text-[#00C2FF] hover:bg-[#1E90FF]/20 font-bold py-2 rounded-lg text-sm transition-colors">
                    View Spot Details
                  </button>
                </div>
              ))}
          ...
          {activeTab === "gear" && (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[
                ...
                <div key={item.name} className="bg-white/5 border border-[#1E90FF]/30 rounded-xl p-4 hover:border-[#1E90FF]/30 transition-all">
                  <div className="text-xs text-[#00C2FF] font-bold mb-1">{item.brand}</div>
                  <div className="text-white font-bold mb-1">{item.name}</div>
                  <div className="text-gray-400 text-xs mb-3">{item.type}</div>
                  <div className="flex items-center justify-between">
                    <div className="text-[#00C2FF] font-bold">{item.price}</div>
                    <div className="text-[#1E90FF] text-xs"> {item.rating}</div>
                  </div>
                  <button className="mt-3 w-full bg-[#1565C0] hover:bg-[#1E90FF] text-white font-bold py-1.5 rounded-lg text-sm transition-colors">
                    Shop Now
                  </button>
                </div>
              ))}
      ...
      {/* CTA */}
      <section className="py-16 px-4 bg-gradient-to-r from-[#1E90FF]/20 to-blue-900/20 border-t border-[#1E90FF]/30">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link href="/signin" className="inline-block bg-[#1565C0] hover:bg-[#1E90FF] text-white font-black text-xl px-10 py-5 rounded-xl transition-all hover:scale-105">
             Start Free — 7 Days on Us
          </Link>
        </div>
      </section>

      <footer className="py-8 px-4 border-t border-[#1E90FF]/30 text-center">
        <p className="text-gray-600 text-sm">Reel Masters · Powered by AthlynX · A Dozier Holdings Group Company</p>
        <Link href="/" className="text-[#00C2FF] text-sm hover:text-[#00C2FF] mt-2 inline-block">← Back to AthlynX Platform</Link>
      </footer>
      <MobileBottomNav />
    </div>
  );
