...
const EVENTS: EliteEvent[] = [
  ...
  },
  {
    ...
    date: "2026-07-22", location: "Laurel, MS", state: "MS", tier: "National",
...
const CATEGORY_COLORS: Record<EventCategory, { bg: string; border: string; text: string; badge: string }> = {
  ...
  "All-Star Game":  { bg: "bg-blue-900/30/30", border: "border-blue-400/40/40", text: "text-blue-300", badge: "bg-blue-900/30" },
  ...
  "Invitational":   { bg: "bg-blue-900/30", border: "border-blue-500/40", text: "text-blue-300", badge: "bg-blue-600" },
  "Clinic":         { bg: "bg-slate-800/60",  border: "border-slate-600/40",  text: "text-slate-300",  badge: "bg-slate-600" },
};

const TIER_BADGE: Record<string, string> = {
  National: "bg-blue-900/30/20 border border-blue-400/40/40 text-blue-300",
    ...
    <PlatformLayout>
      <div className="max-w-7xl mx-auto space-y-6 pb-12 px-2">
        ...
        <div className="bg-gradient-to-r from-[#0d1b3e] via-[#1a3a8f] to-[#0d1b3e] border border-blue-700 rounded-2xl p-6 relative overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="bg-blue-900/30/20 border border-blue-400/40/40 text-blue-300 text-xs font-bold px-2 py-0.5 rounded-full">ELITE EVENTS</span>
            ...
            <div className="flex gap-2">
              <button className="flex items-center gap-2 bg-blue-900/30 hover:bg-blue-900/30 text-white font-bold px-4 py-2.5 rounded-xl text-sm transition-all">
            ...
            <div className="flex gap-2">
              {(["All", "National", "Regional", "State"] as const).map(t => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                    tierFilter === t ? "bg-blue-900/30 text-white" : "bg-slate-900/60 text-slate-400 border border-slate-700 hover:text-white"
                  }`}
                >
                  {t}
                </button>
        ...
        <div className="bg-gradient-to-r from-[#1a3a8f]/60 to-slate-800/60 border border-blue-700/40 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button className="bg-gradient-to-r from-blue-500 to-red-600 hover:from-blue-500 hover:to-red-500 text-white font-black px-6 py-3 rounded-xl text-sm transition-all whitespace-nowrap flex items-center gap-2 shadow-lg">
    ...
    <div className={`${colors.bg} ${colors.border} border rounded-2xl p-4 flex flex-col gap-3 hover:scale-[1.01] transition-all ${featured ? "ring-1 ring-blue-500/20" : ""}`}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
              {featured && <Star className="w-3 h-3 text-blue-300 shrink-0" />}
      ...
      {spotsPercent !== null && (
        <div>
          <div className="flex justify-between text-xs mb-1">
            ...
            <span className={`font-bold ${spotsPercent >= 80 ? "text-red-400" : spotsPercent >= 50 ? "text-blue-300" : "text-green-400"}`}>{spotsPercent}% full</span>
          </div>
          <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${spotsPercent >= 80 ? "bg-red-500" : spotsPercent >= 50 ? "bg-blue-900/30" : "bg-green-500"}`}
      ...
      <div className="flex items-center justify-between gap-2 pt-1 border-t border-white/5">
        <div className="flex items-center gap-2">
          ...
          {event.price && <span className={`text-xs font-bold ${event.price === "Invite Only" ? "text-blue-300" : "text-green-400"}`}>{event.price}</span>}
        </div>
        <button
