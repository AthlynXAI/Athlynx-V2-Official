    ...
    <main className="min-h-screen bg-[#020817] text-white">
      <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.28),transparent_32%),linear-gradient(135deg,#020817_0%,#071a3f_52%,#020617_100%)]">
        <div className="absolute inset-0 opacity-20" aria-hidden="true">
          <div className="absolute right-[-120px] top-16 h-80 w-80 rounded-full bg-cyan-400 blur-3xl" />
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-16 md:py-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <div className="mb-6 inline-flex rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-xs font-black uppercase tracking-[0.28em] text-cyan-100">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#request-playbook" className="rounded-xl bg-cyan-300 px-6 py-3 text-sm font-black text-slate-950 shadow-lg shadow-cyan-500/20 transition hover:bg-cyan-200">
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/30 backdrop-blur">
            <div className="overflow-hidden rounded-[1.5rem] border border-cyan-300/20 bg-[#04152e]">
              <img src="/media/championship-brand/episode-2/scene13-baseball-hero.jpg" alt="AthlynX Baseball — Performance. Identity. Legacy." className="h-64 w-full object-cover object-top opacity-95" />
              <div className="p-6">
                <p className="text-xs font-black uppercase tracking-[0.28em] text-cyan-200">Proof before outreach</p>
      ...
      <section id="proof-system" className="mx-auto max-w-7xl px-6 py-14">
        <div className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-cyan-300">The Smith Standard</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {proofItems.map((item) => (
              <div key={item} className="rounded-2xl border border-cyan-300/15 bg-[#061633] p-5">
      ...
      <section className="border-y border-white/10 bg-white/[0.03]">
        <div className="mx-auto max-w-7xl px-6 py-14">
          <p className="text-sm font-black uppercase tracking-[0.25em] text-cyan-300">Inside Playbook 1.0</p>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-4">
            {playbookSections.map((section) => (
              <article key={section.title} className="rounded-2xl border border-white/10 bg-[#061633] p-5 shadow-xl shadow-black/20">
                <h3 className="text-lg font-black text-cyan-100">{section.title}</h3>
      ...
      <section id="request-playbook" className="mx-auto grid max-w-7xl gap-8 px-6 py-14 lg:grid-cols-[1fr_0.9fr]">
        <div className="rounded-3xl border border-cyan-300/20 bg-gradient-to-br from-[#071b42] to-[#031026] p-6 md:p-8">
          <p className="text-sm font-black uppercase tracking-[0.25em] text-cyan-300">Request the Playbook</p>
          ...
          <form className="mt-8 grid gap-4" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }}>
            ...
            <button type="submit" className="rounded-xl bg-cyan-300 px-6 py-3 text-sm font-black text-slate-950 shadow-lg shadow-cyan-500/20 transition hover:bg-cyan-200">
              Request the Playbook
            </button>
          </form>
          {submitted && (
            <div className="mt-5 rounded-2xl border border-cyan-300/25 bg-cyan-300/10 p-4 text-sm font-bold text-cyan-100">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 md:p-8">
          <p className="text-sm font-black uppercase tracking-[0.25em] text-cyan-300">CRM-ready sequence</p>
          ...
          <div className="mt-6 space-y-4">
            {emailSequence.map((email, index) => (
              <div key={email.subject} className="rounded-2xl border border-white/10 bg-[#061633] p-4">
                <div className="text-sm font-black text-cyan-300">Email {index + 1} · {email.timing}</div>
