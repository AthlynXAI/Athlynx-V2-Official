...
import { Activity, AlertTriangle, CheckCircle2, Hammer, RefreshCw, Smartphone, FileText, PlayCircle, Bell, Trophy } from "lucide-react";
...
function statusBadge(status: string) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    canceled: { label: "Canceled", cls: "bg-blue-900/40/20 text-blue-400 border-blue-500/40/30" },
  };
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" disabled className="gap-2 opacity-60">
              <Trophy className="h-4 w-4" /> iOS (pending)
            </Button>
            ...
            <CardHeader>
              <CardTitle className="text-base text-white flex items-center gap-2">
                ...
                {logQuery.data?.source === "mock" && (
                  <Badge className="bg-blue-900/40/20 text-blue-400 border border-blue-500/40/30">
                    mock
                  </Badge>
                )}
              </CardTitle>
                ...
                <>
                  ...
                  <ul className="text-xs text-slate-300 space-y-1">
                    {logQuery.data.parsed.signals.slice(0, 20).map((s, i) => (
                      <li key={i}>
                        <span className="text-blue-400">[{s.severity}]</span>{" "}
