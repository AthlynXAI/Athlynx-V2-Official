  ...
  return (
    <div className="min-h-screen relative text-white overflow-x-hidden flex items-center justify-center">
      ...
      <div className="relative w-full max-w-[640px] mx-auto px-4 py-8 space-y-10">
        ...
        {position && (
          <div className="text-center">
            <div className="inline-flex flex-col items-center gap-2 bg-gradient-to-br from-cyan-900/80 to-blue-900/80 border-2 border-cyan-400 rounded-3xl px-8 py-5 shadow-2xl">
              <span className="text-cyan-400 text-xs font-black uppercase tracking-widest">Your Waitlist Position</span>
              <span className="text-white font-black text-6xl"># {position}</span>
              <span className="text-cyan-300 text-sm">Share with a friend to move up the list</span>
            </div>
          </div>
        )}

        {/* Share to move up — always show this card */}
        <div className="bg-black/80 backdrop-blur-xl border-2 border-cyan-400/60 rounded-3xl p-6 space-y-4 shadow-2xl">
          <div className="text-center">
            <h3 className="text-cyan-400 font-black text-xl uppercase tracking-wider mb-1">🔗 Share to Move Up</h3>
            <p className="text-gray-300 text-sm mb-4">Each friend who signs up bumps your position up the list.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={copyLink}
              className="flex-1 flex items-center justify-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-black font-black py-3 px-4 rounded-xl transition-all"
            >
