...
const badgeTone: Record<string, string> = {
  ...
  needs_setup: "bg-blue-900/40/15 text-blue-400 border-blue-500/40/30",
  planned: "bg-sky-500/15 text-sky-200 border-sky-400/30",
  critical: "bg-red-500/15 text-red-200 border-red-400/30",
  high: "bg-blue-500/15 text-blue-200 border-blue-400/30",
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-50">
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-6 py-10 lg:px-10">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {providers.map((provider) => (
              <article key={provider.id} className="rounded-2xl border border-slate-700/70 bg-slate-900/70 p-5">
                ...
                {provider.limitation ? <p className="mt-4 text-sm text-blue-400">{provider.limitation}</p> : null}
        ...
        <section className="rounded-3xl border border-blue-500/40/20 bg-blue-900/40/20 p-6">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            {(summary?.nextProviderWork ?? [
              ...
              <div key={item} className="rounded-2xl border border-blue-500/40/10 bg-slate-950/50 p-4 text-sm text-blue-400">
                {item}
              </div>
            ))}
          </div>
