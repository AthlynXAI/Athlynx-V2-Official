...
const AGENTS = [
  ...
  },
  {
    ...
    humanPhone: "Book a call",
    status: "active",
    purpose: "Real-time NIL analysis, EPX scoring, recruiting intelligence, scouting reports, and content generation.",
...
const IP_ASSETS = [
  ...
  { name: "EPX Scoring Algorithm", type: "Algorithm / Trade Secret", owner: "AthlynXAI Corporation", registered: "Proprietary — not publicly disclosed", law: "Texas Uniform Trade Secrets Act" },
  ...
  return (
    <PlatformLayout title="Agent Ownership & Security">
      <div className="max-w-5xl mx-auto px-2 py-4 space-y-5 pb-24">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-[#0a1628] to-[#1a3a8f] border border-[#1E90FF]/40 rounded-2xl p-6">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 bg-[#1E90FF]/20 border border-[#1E90FF]/40 rounded-2xl flex items-center justify-center shrink-0">
              <Shield className="w-7 h-7 text-[#1E90FF]" />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className="text-[#1E90FF] text-xs font-black uppercase tracking-widest">CLASSIFIED — ADMIN ONLY</span>
                <span className="bg-[#1E90FF] text-white text-[9px] font-black px-2 py-0.5 rounded-full">MASTER ADMIN</span>
              </div>
          ...
          <div className="mt-4 bg-[#0d1b3e] border border-blue-800 rounded-xl p-4 flex items-center gap-4">
            <Crown className="w-8 h-8 text-[#00C2FF] shrink-0" />
            <div>
              <div className="text-[#00C2FF] text-xs font-black uppercase tracking-wider">Master Admin — Human Identity Verified</div>
            ...
            <div className="ml-auto text-right shrink-0">
              <div className="text-[#00C2FF] text-xs font-bold flex items-center gap-1 justify-end"><CheckCircle2 className="w-3 h-3" /> VERIFIED OWNER</div>
              <div className="text-blue-400 text-xs">Sole kill-switch authority</div>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            <span className="bg-[#00C2FF]/20 border border-[#00C2FF]/30 text-[#00C2FF] text-[10px] font-black px-2 py-1 rounded-full"> {AGENTS.length} AI Agents Active</span>
            ...
            <span className="bg-blue-500/20 border border-blue-500/30 text-[#00C2FF] text-[10px] font-black px-2 py-1 rounded-full"> {IP_ASSETS.length} IP Assets Documented</span>
            <span className="bg-[#1E90FF]/20 border border-[#1E90FF]/30 text-[#1E90FF] text-[10px] font-black px-2 py-1 rounded-full"> {SECURITY_RULES.length} Security Rules Enforced</span>
          </div>
        </div>
        ...
        {activeTab === "agents" && (
          <div className="space-y-4">
            <div className="bg-blue-900/20 border border-blue-700/40 rounded-xl p-3 text-xs text-[#00C2FF]">
            ...
            {AGENTS.map(agent => {
              ...
              return (
                <div key={agent.id} className={`bg-[#0d1b3e] border rounded-2xl overflow-hidden ${isKilled ? "border-[#1E90FF] opacity-70" : "border-blue-900/60"}`}>
                  <div className={`bg-gradient-to-r ${isKilled ? "from-[#1E90FF] to-gray-900" : agent.color} p-4 flex items-start justify-between`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-full ${agent.securityLevel === "HIGH" ? "bg-[#1E90FF] text-white" : "bg-blue-700 text-white"}`}>{agent.securityLevel}</span>
                        </div>
                        <div className="text-white/70 text-xs">{agent.role}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${isKilled ? "bg-[#1E90FF] text-white" : "bg-[#00C2FF]/30 text-[#00C2FF] border border-[#00C2FF]/30"}`}>
                        {isKilled ? " KILLED" : " ACTIVE"}
                      </span>
                      {isKilled
                        ? <button onClick={() => handleReactivate(agent.id, agent.name)} className="flex items-center gap-1 bg-[#00C2FF] hover:bg-[#00C2FF] text-white text-[10px] font-black px-2 py-1 rounded-lg"><RefreshCw className="w-3 h-3" />Restore</button>
                        : <button onClick={() => handleKillSwitch(agent.id, agent.name)} className="flex items-center gap-1 bg-[#1E90FF] hover:bg-[#1E90FF] text-white text-[10px] font-black px-2 py-1 rounded-lg"><XCircle className="w-3 h-3" />Kill</button>
                      }
                    </div>
                  </div>
                  <div className="p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="bg-[#060d1f] border border-blue-900/40 rounded-xl p-3">
                        <div className="flex items-center gap-1 mb-1"><User className="w-3 h-3 text-[#00C2FF]" /><span className="text-[#00C2FF] text-[10px] font-black uppercase">Human Identity</span></div>
                        <div className="text-white text-sm font-bold">{agent.humanOwner}</div>
                        <div className="text-blue-400 text-xs">{agent.humanEmail}</div>
                      </div>
                      <div className="bg-[#060d1f] border border-blue-900/30 rounded-xl p-3">
                        <div className="flex items-center gap-1 mb-1"><Key className="w-3 h-3 text-[#00C2FF]" /><span className="text-[#00C2FF] text-[10px] font-black uppercase">IP Owner</span></div>
                    ...
                    <div className="bg-[#1E90FF]/30 border border-[#1E90FF]/40 rounded-xl p-3 flex items-start gap-2">
                      <Power className="w-4 h-4 text-[#1E90FF] shrink-0 mt-0.5" />
                      <div>
                        <div className="text-[#1E90FF] text-[10px] font-black uppercase mb-0.5">Kill Switch</div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${conn.status === "active" ? "bg-[#00C2FF] text-white" : "bg-blue-700 text-white"}`}>{conn.status.toUpperCase()}</span>
                      <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-blue-900 text-[#00C2FF] flex items-center gap-1"><Lock className="w-2 h-2" />SECURED</span>
                    </div>
                    ...
                    <div className="bg-[#060d1f] border border-blue-900/40 rounded-lg p-2 mb-2">
                      <div className="text-[#00C2FF] text-[10px] font-bold mb-0.5">Security Note</div>
                      <div className="text-blue-300 text-xs">{conn.securityNote}</div>
                    </div>
                    <div className="bg-[#1E90FF]/30 border border-[#1E90FF]/40 rounded-lg p-2 flex items-center gap-2">
                      <Power className="w-3 h-3 text-[#1E90FF] shrink-0" />
                      <div><span className="text-[#1E90FF] text-xs font-bold">Kill Switch: </span><span className="text-blue-300 text-xs">{conn.killSwitch}</span></div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {activeTab === "ip" && (
          <div className="space-y-4">
            <div className="bg-gradient-to-r from-blue-950/40 to-[#0d1b3e] border border-blue-700/40 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-2">
                <Building2 className="w-6 h-6 text-[#00C2FF]" />
                <div>
                  <div className="text-[#00C2FF] text-xs font-black uppercase tracking-wider">Legal IP Declaration</div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      ...
                      <span className="text-[10px] bg-blue-900/30 text-[#00C2FF] px-2 py-0.5 rounded-full border border-blue-800/30">{asset.owner}</span>
                    </div>
                    <div className="text-blue-400 text-xs mb-1">Registration: {asset.registered}</div>
                    <div className="text-[#00C2FF] text-xs font-semibold">{asset.law}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        {activeTab === "security" && (
          <div className="space-y-4">
            ...
            {SECURITY_RULES.map((rule, i) => (
              <div key={i} className={`bg-[#0d1b3e] border rounded-xl p-4 ${rule.severity === "CRITICAL" ? "border-[#1E90FF]/50" : "border-blue-700/40"}`}>
                <div className="flex items-start gap-3">
                  <AlertTriangle className={`w-5 h-5 shrink-0 mt-0.5 ${rule.severity === "CRITICAL" ? "text-[#1E90FF]" : "text-[#00C2FF]"}`} />
                  <div>
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-white font-black text-sm">{rule.rule}</span>
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${rule.severity === "CRITICAL" ? "bg-[#1E90FF] text-white" : "bg-blue-700 text-white"}`}>{rule.severity}</span>
                    </div>
            ...
            <div className="bg-gradient-to-r from-[#1E90FF]/30 to-[#0d1b3e] border border-[#1E90FF]/40 rounded-xl p-4 text-center">
              <div className="text-[#1E90FF] text-xs font-black uppercase tracking-wider mb-1">Emergency Protocol</div>
