export default function AthletePlaybookEpisode1() {
  ...
  return (
    <main className="min-h-screen bg-[#020817] text-white">
      <section className="relative overflow-hidden border-b border-sky-400/20 px-5 py-16 sm:px-8 lg:px-16">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <p className="mb-4 text-sm font-bold uppercase tracking-[0.35em] text-sky-200">AthlynXAI Memorial Day Launch</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="/athlynxai-os" className="rounded-full bg-sky-400 px-6 py-3 text-sm font-black uppercase tracking-wide text-slate-950 shadow-lg shadow-sky-500/25">
                Explore AthlynXAI OS
              </a>
              <a href="/athlete-playbook" className="rounded-full border border-sky-200/60 px-6 py-3 text-sm font-black uppercase tracking-wide text-white">
                Athlete Playbook
              </a>
              {continuityLinks.map((link) => (
                <a key={link.href} href={link.href} target="_blank" rel="noreferrer" className="rounded-full border border-white/30 px-6 py-3 text-sm font-black uppercase tracking-wide text-white hover:border-sky-200">
                  {link.label}
                </a>
              ))}
            </div>
          </div>
          <div className="rounded-[2rem] border border-sky-300/25 bg-black/40 p-3 shadow-2xl shadow-sky-950/60">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-5 py-12 sm:px-8 lg:grid-cols-4 lg:px-16">
        {launchStats.map((item) => (
          <div key={item.label} className="rounded-3xl border border-sky-300/20 bg-white/5 p-6">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-sky-200">{item.label}</p>
