      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-14 pb-10">
        <div className="flex justify-center mb-4">
          <div className="flex items-center gap-2 bg-[#00C2FF]/20 border border-[#00C2FF]/30 rounded-full px-4 py-2">
            ...
            <span className="text-[#00C2FF] text-sm font-semibold">AthlynX CAREER HUB</span>
          </div>
        </div>
        <h1 className="text-4xl sm:text-5xl font-black text-center mb-3">
          Life After <span className="bg-gradient-to-r from-[#00C2FF] to-emerald-400 bg-clip-text text-transparent">The Game</span>
        </h1>
        ...
        {/* Warning Stats */}
        <div className="bg-[#1E90FF]/30 border border-[#1E90FF]/20 rounded-2xl p-4 mb-8 max-w-3xl mx-auto">
          <h3 className="text-[#1E90FF] font-bold text-sm text-center mb-3">The Reality No One Talks About</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {STATS.map((s, i) => (
              <div key={i} className="text-center">
                <div className="text-lg font-black text-[#1E90FF]">{s.value}</div>
            ...
            {CAREER_PATHS.map((path, i) => {
              ...
              return (
                <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-5 hover:bg-white/10 transition-all cursor-pointer">
                  <Icon className="w-8 h-8 text-[#00C2FF] mb-3" />
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-[#00C2FF] text-xs font-bold">{path.avgSalary}</div>
                    </div>
                    <div className={`text-xs font-bold px-2 py-1 rounded-full ${
                      path.demand === "Explosive" ? "bg-blue-500/20 text-[#00C2FF]" :
                      path.demand === "Very High" ? "bg-[#00C2FF]/20 text-[#00C2FF]" :
              ...
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-[#00C2FF] text-sm font-semibold">{mentor.sport}</p>
                  ...
                  <div className="text-right">
                    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${mentor.available ? "bg-[#00C2FF]/20 text-[#00C2FF]" : "bg-slate-500/20 text-slate-400"}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${mentor.available ? "bg-[#00C2FF]" : "bg-slate-400"}`} />
                ...
                <button className="w-full flex items-center justify-center gap-2 bg-[#00C2FF] hover:bg-[#00C2FF] text-white text-sm font-bold py-2.5 rounded-xl transition-all">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {TRANSITION_RESOURCES.map((res, i) => {
                ...
                return (
                  <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-5 hover:bg-white/10 transition-all cursor-pointer">
                    <Icon className="w-7 h-7 text-[#00C2FF] mb-3" />
        ...
        <div className="mt-12 bg-gradient-to-r from-[#00C2FF]/50 to-emerald-900/50 border border-[#00C2FF]/20 rounded-2xl p-6 sm:p-8 text-center">
          <TrendingUp className="w-10 h-10 text-[#00C2FF] mx-auto mb-3" />
