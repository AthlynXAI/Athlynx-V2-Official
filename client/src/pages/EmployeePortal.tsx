...
const TEAM = [
  ...
  { name: "Lee Marshall", role: "Co-Host · VP Sales & Partnerships", email: "lmarshall@athlynx.ai", focus: "Revenue, Athlete NIL", avatar: "L", color: "#059669" },
      ...
      <div className="space-y-4 pb-20 lg:pb-4">
                ...
                {[
                  ...
                  <div key={i} className="flex items-center justify-between py-2 border-b border-blue-900/40">
                    ...
                    <span className={`text-xs font-black px-2 py-0.5 rounded-full ${
                      sys.status === "LIVE" ? "bg-green-900/40 text-green-400 border border-green-700" : "bg-blue-900/30/40 text-blue-300 border border-blue-400/40"
                    }`}>{sys.status}</span>
                  </div>
                ))}
              </div>
        ...
        {activeTab === "team" && (
          <div className="space-y-3">
            ...
            {/* Tony's profile listing stays visible per OWNERSHIP.md directive;
                his active-access affordances (email links) are gated below. */}
            {TEAM.map((member, i) => (
              <div key={i} className="bg-[#1a3a8f] border border-blue-900 rounded-xl p-4 flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  {/* Email row only renders when access is active. When access
                      is on hold, the row is omitted entirely — no public
                      "Suspended" label so the hold stays invisible. */}
                  {!isSuspended(member.name) && (
                    <div className="text-blue-500 text-xs mt-1">{member.email}</div>
                  )}
                </div>
                <div className="flex flex-col gap-1">
                  {/* Action buttons hidden silently for members on hold. */}
                  {!isSuspended(member.name) && (
                    <>
