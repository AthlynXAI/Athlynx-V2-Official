...
// Email only. Instagram/TikTok handles are captured as signup metadata until OAuth review is complete.
...
export function AuthShell({ mode }: { mode: Mode }) {
  ...
  // Supabase OAuth redirects to /auth/callback automatically — syncSupabaseUser is called there
  ...
  async function door(provider: "google" | "apple" | "facebook") {
    ...
    setBusy(provider);
    try {
      // Supabase OAuth redirects to /auth/callback — syncSupabaseUser called there
      if (provider === "google") await signInWithGoogle();
      else if (provider === "apple") await signInWithApple();
      else await signInWithFacebook();
      // Page will redirect via Supabase OAuth flow
    } catch (err: any) {
      const msg = err?.message ?? "";
