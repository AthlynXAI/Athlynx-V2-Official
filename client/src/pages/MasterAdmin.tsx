...
import { Send, Users, Megaphone, Shield, Search, Bell, CheckCircle, AlertTriangle, BarChart2, Crown, Trophy } from "lucide-react";
            ...
            {usersData?.users?.map((u: any) => (
              <div key={u.id} className="flex items-center gap-3 p-3 rounded-xl bg-[#0d0d2a] border border-blue-950/30 hover:border-blue-800/40 transition-all">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-white font-bold text-sm truncate flex items-center gap-1.5">
                    ...
                    {isMasterAdmin(u.email) && (
                      <Trophy className="w-3.5 h-3.5 text-blue-400 shrink-0" aria-label="Master Admin" />
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {isMasterAdmin(u.email) ? (
                    <Badge className="bg-blue-900/40/20 text-blue-400 border-blue-500/40/40 text-xs">Master Admin</Badge>
                  ) : u.role === "admin" && (
