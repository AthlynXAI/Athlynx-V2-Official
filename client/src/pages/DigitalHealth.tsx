    ...
    <div className="min-h-screen bg-gradient-to-br from-[#07111f] via-[#0b1b34] to-[#102a4f] text-white">
      ...
      <main className="px-4 py-16 md:py-20">
        <section className="mx-auto mb-14 max-w-5xl text-center">
          <span className="mb-5 inline-block rounded-full border border-cyan-300/20 bg-cyan-400/10 px-4 py-1 text-sm font-bold text-cyan-200">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link href="/glucoathlete" className="rounded-xl bg-cyan-400 px-6 py-3 font-black text-[#07111f] transition hover:bg-cyan-300">
        ...
        <section className="mx-auto mb-10 max-w-7xl rounded-[2rem] border border-cyan-300/25 bg-gradient-to-r from-cyan-400/15 via-blue-500/10 to-emerald-400/10 p-7 text-left shadow-2xl shadow-cyan-950/30">
          <p className="text-sm font-black uppercase tracking-[0.22em] text-cyan-200">Platform Highlight · Founder Tested</p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            {["Founder tested", "LibreView proof", "Universal channel", "Human escalation"].map((item) => (
              <div key={item} className="rounded-2xl border border-white/10 bg-black/25 p-4 font-black text-cyan-100">{item}</div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto mb-14 grid max-w-7xl gap-5 md:grid-cols-2 xl:grid-cols-3">
          {featureAreas.map((feature) => (
            <article key={feature.name} className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-xl shadow-black/10 transition hover:border-cyan-300/40">
              <div className="mb-4 flex items-start gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-cyan-400/10 text-3xl">{feature.icon}</div>
                <div>
        ...
        <section className="mx-auto mb-8 max-w-5xl rounded-2xl border border-blue-500/40/30 bg-blue-900/40/10 p-6 text-left">
          <h2 className="mb-3 text-2xl font-black text-blue-400">Medical Safety Boundary</h2>
          <p className="leading-relaxed text-blue-400/90">
