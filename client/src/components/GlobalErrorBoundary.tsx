...
export class RouteErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    // Surface to global window for /admin/diagnostics page and Sentry (if wired)
    try {
      const w = window as unknown as { __ATHLYNX_LAST_ERROR__?: { message: string; stack?: string; componentStack?: string; at: string } };
      w.__ATHLYNX_LAST_ERROR__ = {
    ...
    // Send to /api/client-error so server can log it server-side too
    try {
  ...
  render() {
    if (this.state.hasError) {
      const showDetails = typeof window !== "undefined"
        && (window.location.search.includes("debug=1") || localStorage.getItem("athlynx_debug") === "1");
      ...
      return (
        <div style={{
          ...
          <div style={{ fontSize: "48px", marginBottom: "16px", color: "#1E90FF" }}>⚡</div>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            ...
            <button
              ...
              style={{ background: "rgba(255,255,255,0.08)", border: "1px solid #1E90FF", borderRadius: "10px", color: "#fff", fontSize: "14px", fontWeight: "700", padding: "12px 24px", cursor: "pointer" }}
            >
              Try Again
            </button>
          </div>
