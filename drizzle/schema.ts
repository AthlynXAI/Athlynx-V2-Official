...
export const users = pgTable("users", {
  ...
  // Build Jun-06-2026 — HIPAA + AthlynX Medical Data Waiver Consent
  // Athletes must sign all three documents before accessing the platform.
  hipaaConsentSigned: smallint("hipaaConsentSigned").default(0).notNull(),
  medicalWaiverSigned: smallint("medicalWaiverSigned").default(0).notNull(),
  athlynxDataWaiverSigned: smallint("athlynxDataWaiverSigned").default(0).notNull(),
  consentSignedAt: timestamp("consentSignedAt"),
  consentIpAddress: varchar("consentIpAddress", { length: 64 }),
  consentVersion: varchar("consentVersion", { length: 32 }).default("1.0"),
});
...
// ─── /api/auth/me AUDIT LOG (Build 51 · Jun 3, 2026) ─────────────────────────
// Append-only structured log of every /api/auth/me request. Powers the
// Auth Audit Dashboard at /admin/audit/auth-me. No raw cookies, headers,
// or tokens are ever stored — only sanitized flags.
export const authMeEvents = pgTable("auth_me_events", {
  id: serial("id").primaryKey(),
  occurredAt: timestamp("occurred_at", { withTimezone: true }).defaultNow().notNull(),
  status: integer("status").notNull(),
  route: text("route").default("/api/auth/me").notNull(),
  sessionId: text("session_id"),
  userId: integer("user_id"),
  userEmail: text("user_email"),
  ip: text("ip"),
  userAgent: text("user_agent"),
  authProvider: text("auth_provider").default("unknown").notNull(),
  tokenState: text("token_state").default("unknown").notNull(),
  failureReason: text("failure_reason"),
  headersPresent: json("headers_present").default({}).notNull(),
  origin: text("origin"),
  referer: text("referer"),
  isLogoutMarker: boolean("is_logout_marker").default(false).notNull(),
  metadata: json("metadata").default({}).notNull(),
});

export type AuthMeEvent = typeof authMeEvents.$inferSelect;
export type InsertAuthMeEvent = typeof authMeEvents.$inferInsert;
