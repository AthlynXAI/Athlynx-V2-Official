/**
 * SDK Server — AthlynX
 * Session-cookie based authentication via Supabase.
 */
...
class SDKServer {
  ...
  async getUserInfoWithJwt(jwtToken: string): Promise<GetUserInfoWithJwtResponse> {
    try {
      // SECURITY: verify signature before trusting any claims (Aikido Critical fix)
      const secretKey = this.getSessionSecret();
      const { payload: decoded } = await jwtVerify(jwtToken, secretKey, {
        algorithms: ["HS256"],
      });
