...
export async function runSocialQueueGuardrail(): Promise<SocialQueueGuardrailResult> {
  ...
  try {
    ...
    if (result.socialContent === 0) failures.push("social_content has zero active rows");
    if (result.scheduledToday === 0) failures.push("social_posts has zero scheduled rows for today");
