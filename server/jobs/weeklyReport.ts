/**
 * Weekly Profile Growth Report — AthlynX
...
// Deliver the weekly signup digest to all canonical owner inboxes.
const OWNER_EMAILS = [
...
export async function sendWeeklyReport(): Promise<void> {
  try {
    ...
    const methodRows = stats.topLoginMethods.length > 0
      ...
      : `<tr><td colspan="2" style="padding:10px 16px;color:#475569;text-align:center;font-size:13px;">No profile claims this week</td></tr>`;
...
<tr><td style="background:linear-gradient(135deg,#0066ff,#00c2ff);padding:28px 32px;">
  ...
  <div style="font-size:12px;color:rgba(255,255,255,0.85);letter-spacing:5px;margin-top:4px;">WEEKLY PROFILE GROWTH REPORT</div>
</td></tr>
<tr><td style="padding:32px;">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
    <tr>
      ...
      <td width="33%" style="text-align:center;padding:16px;background:#0a1628;border-radius:12px;">
        ...
        <div style="font-size:12px;color:#94a3b8;margin-top:4px;">TOTAL ATHLETE PROFILES</div>
      </td>
    </tr>
  </table>
  <h3 style="color:#fff;font-size:15px;margin:0 0 12px;">Profile Claim Methods This Week</h3>
  ...
  <div style="text-align:center;">
    <a href="https://athlynx.ai/admin" style="display:inline-block;background:linear-gradient(135deg,#0066ff,#00c2ff);color:#fff;font-weight:900;font-size:14px;padding:12px 32px;border-radius:50px;text-decoration:none;">VIEW GROWTH DASHBOARD</a>
  </div>
</td></tr>
<tr><td style="background:#060d1f;padding:20px;text-align:center;border-top:1px solid #1e3a6e;">
  ...
  <p style="color:#334155;font-size:11px;margin:6px 0 0;">This report is sent every Sunday at 8:00 AM CST</p>
    ...
    const text = `AthlynX Weekly Profile Growth Report — ${dateRange}\n\nNew profile claims this week: ${stats.thisWeek}\nLast week: ${stats.lastWeek}\nTotal athlete profiles: ${stats.total}\n\nView growth dashboard: https://athlynx.ai/admin`;

    await Promise.all(
      OWNER_EMAILS.map(to =>
        sendEmail({
          to,
          subject: `📊 AthlynX Weekly Profile Growth Report: ${stats.thisWeek} new profile claim${stats.thisWeek !== 1 ? 's' : ''} (${dateRange})`,
          html,
          text,
        })
      )
    );
    console.log(`[WeeklyReport] Sent to ${OWNER_EMAILS.length} addresses — ${stats.thisWeek} new profile claims this week, ${stats.total} total athlete profiles`);
  } catch (err) {
