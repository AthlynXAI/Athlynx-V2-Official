...
import { registerTransferPortalRoutes } from "../server/routes/transferPortal";
...
import { writeAuthMeEvent } from "../server/_core/authAuditLog";
import { sdk } from "../server/_core/sdk";
import { registerAuthMeAuditRoutes } from "../server/routes/audit-auth-me";
...
// Build 51 — /api/auth/me is now a REAL auth probe with structured audit logging.
// 200 when a valid session cookie is present, 401 otherwise. Every call writes
// one row to auth_me_events (sanitized — never raw cookies/tokens) so the
// Auth Audit Dashboard at /admin/audit/auth-me can group by session, UA, IP,
// and provider, and flag 200→401 transitions that occur without a logout.
async function handleAuthMe(req: Request, res: Response, route: string) {
  try {
...
app.get("/api/auth/me", (req: Request, res: Response) => { void handleAuthMe(req, res, "/api/auth/me"); });
app.get("/api/me",       (req: Request, res: Response) => { void handleAuthMe(req, res, "/api/me"); });

// Logout marker — when the client hits this we record a logout row so the
// dashboard can distinguish intentional 200→401 transitions from suspicious
// ones (header misconfig / token expiry mid-session).
app.post("/api/auth/logout-marker", (req: Request, res: Response) => {
  res.status(204).end();
  void writeAuthMeEvent(req, { status: 200, isLogoutMarker: true, metadata: { route: "/api/auth/logout-marker" } });
});

// Admin-only audit API powering the /admin/audit/auth-me dashboard.
registerAuthMeAuditRoutes(app);

// Client-side error sink — receives runtime exceptions from GlobalErrorBoundary /
// RouteErrorBoundary so we can debug crashes without screenshots. Never throws,
// never blocks, always 204. The log line is searchable via `vercel logs --search athlynx-client-error`.
...
registerTransferPortalRoutes(app);
