...
export async function getDb() {
  ...
  // Try Neon (primary) first, then legacy DATABASE_URL, then PlanetScale (backup).
  // NEON_DATABASE_URL is the canonical direct Neon connection string used by
  // cron/API routes so production no longer depends on a brittle Pipedream
  // account configuration for database access.
  const urls = [
  ...
  for (const { url, label } of urls) {
    try {
      ...
      _pool = new Pool({
        ...
        // Note: this is a node-postgres-specific option — safe to leave even
        // if the driver version doesn't read it.
