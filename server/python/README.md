# AthlynXAI — Python Backend Engine

## Overview
This is the Python implementation of the AthlynXAI Layer Cake backend engine.
It mirrors the Julia implementation in `../julia/athlynxai_engine.jl` and
...
## Usage
```bash
# Python 3.10+, no external dependencies for core engine
python server/python/athlynxai_engine.py
```
...
- `dataclass` + `Enum` pattern mirrors the TypeScript type system in `shared/schema.ts`
